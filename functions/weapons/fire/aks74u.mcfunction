...
# anims
playanimation @s[tag=silencer] animation.aks74u.fire.silenced fire 0 "!q.is_item_name_any('slot.weapon.mainhand', 0, 'trenbankai:aks74u')"
playanimation @s[tag=!silencer] animation.aks74u.fire fire 0 "!q.is_item_name_any('slot.weapon.mainhand', 0, 'trenbankai:aks74u')"

# recoil
camerashake add @s[tag=sneaking] 0.02 0.18 rotational
camerashake add @s[tag=!sneaking] 0.07 0.18 rotational

# core
scoreboard players remove @s[scores={aks74u=1..}] aks74u 1
function effect/shot_light
event entity @s trenbankai:aks74u_fire
